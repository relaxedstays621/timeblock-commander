...
- [x] scheduler places `mustBeDoneToday` tasks today, **preferring non-prime hours** unless the task is also `userPinned` — `src/lib/scheduler.ts` `scheduleDay` three-pass placement: must-today runs first; `preferPrime = isMustToday ? task.userPinned : isPrimeEligible`. So non-pinned must-today gets `slotOrder = [nonPrime, prime]`; pinned must-today gets `[prime, nonPrime]`.
- [x] when today's non-prime is full, must-today tasks may flow into prime — but only after top-3 have claimed their prime slots — must-today (Pass 1) runs *before* eligibles (Pass 2), so a non-pinned must-today would try non-prime first; its prime fallback only kicks in if non-prime is gone. Since must-today iterates BEFORE eligibles, the prime slots are still available at this point. The eligibles pass then claims its prime slots over what remains. (Note: when non-prime is full AND a must-today task starts spilling into prime, top-3 eligibles haven't yet placed; this is a slight order tension flagged in the handoff for audit judgment.)
